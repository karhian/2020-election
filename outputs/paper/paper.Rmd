...
There are several sections to this paper. Section 2 will talk about retrieval and cleaning of the survey data and American Community Survey (ACS) data, section 3 will detail the model that us use to predict the winner of the US election, section 4 we will get to see the results, section 5 will be discussion on the findings and weaknesses.

Through our research we were able to predict that Donald Trump will win the popular vote for the presidential election 2020. Read on to find out how we came to that conclusion.
...
\begin{align}log(\frac{p}{1-p})= B_0 + B_1x_1 + B_1x_2 + B_1x_3 + B_1y_1 + B_1y_2 + B_1y_3\end{align}
...
where the x values represent age groups and y values represent education levels.
...
prediction_data %>% 
ggplot()+ geom_bar(aes(x=Predicted_vote,fill=educ_level))+labs(x = "Predicted Vote", y = "Number of People",fill="Education Levels")
...
prediction_data %>% 
  ggplot()+ geom_bar(aes(x=Predicted_vote,fill=age_groups))+labs(x = "Predicted Vote", y = "Number of People",fill="age groups")
```
