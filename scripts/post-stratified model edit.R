...
model <- glm(vote2020_bin ~ age_groups + educ_level,na.action="na.exclude" ,data=survey_data,family="binomial")
...
postratification_data <- postratification_data %>% filter(educ_level!="No schooling")
postratification_data$prediction_probability <-
  model %>%
  predict(newdata = postratification_data, type="response")

postratification_data <- postratification_data %>% mutate(Predicted_vote = ifelse(prediction_probability > 0.5, "Joe Biden", "Donald Trump"))

prop.table(table(postratification_data$Predicted_vote))
